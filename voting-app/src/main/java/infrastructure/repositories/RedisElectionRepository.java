...
public class RedisElectionRepository implements ElectionRepository {
    ...
    public Election findById(String id) {
        ...
        List<Candidate> candidates = sortedSetCommands.zrange("election:" + id, 0, -1).stream().map(Candidate::new).toList();
    ...
    public List<Election> findAll() {
        ...
        return keyCommands.keys("election:*").stream().map(id -> findById(id.replace("election:", ""))).toList();
    }
